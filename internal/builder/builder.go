...
func Build(root string) error {
	...
	if err := runGradle(root, "assembleDebug"); err != nil {
		return os.WriteFile(filepath.Join(out, "README.txt"), []byte(fmt.Sprintf("Debug build is configured, but Gradle could not produce an APK in this environment: %v\n", err)), 0o644)
	}
...
func Release(root string) error {
	...
	if err := runGradle(root, "assembleRelease", "bundleRelease"); err != nil {
		return os.WriteFile(filepath.Join(out, "README.txt"), []byte(fmt.Sprintf("Release build is configured, but Gradle could not produce artifacts in this environment: %v\n", err)), 0o644)
	}
